...
pub fn print_hardware_info(console: &mut impl fmt::Write, info: &BootInfo) {
    ...
    write!(
        console,
        "{:<16} {:#08X}\n",
        "Memory start", info.first_memory_region.start
    )
    .ok();
    write!(
        console,
        "{:<16} {} MiB\n",
        "Memory size",
        info.first_memory_region.size / 1024 / 1024
    )
    .ok();
    write!(console, "{:-<48}\n", "").ok();
...
fn write_region_table_header(output: &mut impl Write) -> fmt::Result {
    writeln!(
        output,
        "{:<12} {:>18} {:>18} {:>15}",
...
fn write_region(output: &mut impl Write, name: &str, start: usize, end: usize) -> fmt::Result {
    let size = end
        .checked_sub(start)
